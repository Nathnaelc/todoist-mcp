  ...
  'get_tasks',
  {
    ...
    inputSchema: {
      filter: z.string().optional().describe(
        'Todoist filter string e.g. "overdue", "today", "p1", "#Work"'
      ),
    },
  },
  async ({ filter }) => {
...
server.registerTool(
  ...
  async ({ parentId, content, dueDate }) => {
    const task = await todoist.addTask(
      dueDate ? { content, parentId, dueDate } : { content, parentId }
    )
...
server.registerTool(
  'reschedule_task',
  {
    ...
    inputSchema: {
      taskId: z.string().describe('The Todoist task ID'),
      dueString: z.string().describe('Due date string — supports natural language and recurrence e.g. "every day", "every mon,wed,fri", "tomorrow", "2026-03-04"'),
    },
  },
  async ({ taskId, dueString }) => {
    await todoist.updateTask(taskId, { dueString })
    return { content: [{ type: 'text', text: `Task ${taskId} rescheduled to ${dueString}` }] }
  }
)
...
server.registerTool(
  'create_task',
  {
    ...
    inputSchema: {
      ...
      priority: z.number().int().min(1).max(4).optional().describe('Priority 1 (urgent) to 4 (normal)'),
      dueString: z.string().optional().describe('Due date string — supports natural language and recurrence e.g. "every day", "every mon,wed,fri", "tomorrow", "2026-03-04"'),
      labels: z.array(z.string()).optional().describe('Label names to apply'),
      durationMinutes: z.number().int().positive().optional().describe('Estimated duration in minutes'),
      sectionId: z.string().optional().describe('Section ID to place the task in'),
    },
  },
  async ({ content, description, projectId, parentId, priority, dueString, labels, durationMinutes, sectionId }) => {
    const base = { content, description, projectId, parentId, priority, labels, ...(sectionId ? { sectionId } : {}) }
    const withDuration = durationMinutes
      ? { ...base, duration: durationMinutes, durationUnit: 'minute' as const }
      : base
    const args = dueString ? { ...withDuration, dueString } : withDuration
...
server.registerTool(
  ...
  async ({ taskId }) => {
    ...
    const formatted = comments.map(c =>
      `[${c.postedAt}] (uid:${c.postedUid}): ${c.content}`
    ).join('\n')
...
server.registerTool(
  ...
  async () => {
    ...
    const formatted = labels.map(l =>
      `[${l.id}] ${l.name} | color: ${l.color}${l.isFavorite ? ' ★' : ''}`
    ).join('\n')
